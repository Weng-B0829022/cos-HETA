...
/* dst &= ~src （in-place 差集，從 dst 移除所有 src 中存在的元素） */
 ...
 *
 * 注意：此函式使用「BFS 排除 t 整個節點」的鄰域定義；如要與 Python HETA
 * 完全一致（Python 只排除「直接從 s 走 s-t 這一步」、t 仍可作為路徑中繼），
 * 請改用 compute_all_link_ratios（見下）。本函式保留供 auto.c 計算單層
 * R^1（k=1 時兩種定義等價）使用。
 */
...
/* ────────────────────────────────────────────────────────────
 * Ego Table — 預先計算所有節點的 0~(layers-1) 階「不受限」鄰域 bitset
 *
 * 對應 Python HETA 的 generate_ego_graph()：
 *   ego(u, 0) = {u}
 *   ego(u, r) = ego(u, r-1) ∪ ⋃_{ng ∈ N(u)} ego(ng, r-1)
 *
 * 注意：這是「不排除任何節點」的標準封閉球，計算後可被多條邊重複查表。
 * 排除特定節點（例如邊的另一端）的邏輯在 compute_all_link_ratios 內處理。
 * ──────────────────────────────────────────────────────────── */
typedef struct {
    Bitset **bitsets;   /* bitsets[u * n_layers + r] = ego(u, r) bitset */
    int      n;         /* 節點數                                     */
    int      n_layers;  /* 計算層數（r = 0..n_layers-1 都已存）         */
} EgoTable;

/* 為圖 g 建立並填入 ego table（layers = 後續查詢需要的最大層數） */
...
/* ────────────────────────────────────────────────────────────
 * compute_all_link_ratios — Python HETA 風格的環狀分層 ratio 計算
 *
 * 對每條邊 (s, t)，依序計算 layer 1..layers 的 ratio：
 *   acc_s, acc_t  = 累積已涵蓋節點（每邊各自重置為空）
 *   for l = 1..layers:
 *     get_ego(s, t, l) = ⋃_{ng ∈ N(s), ng ≠ t} ego(ng, l-1) − {s}
 *     ring_s[l]        = get_ego(s, t, l) − acc_s − {s, t}
 *     （t 側對稱）
 *     common = (ring_s[l] ∩ ring_t[l])
 *            ∪ (ring_s[l] ∩ ring_t[l-1])
 *            ∪ (ring_s[l-1] ∩ ring_t[l])
 *     denom  = min(|ring_s[l]|,|ring_t[l]|)
 *            + min(|ring_s[l]|,|ring_t[l-1]|)
 *            + min(|ring_s[l-1]|,|ring_t[l]|)
 *     ratio  = |common| / denom（denom=0 或 |common|=0 時為 0）
 *     acc_s ∪= ring_s[l]; acc_t ∪= ring_t[l]
 *
 * 邊以 CSR 順序枚舉（u<v），edge_idx 為 0..m-1。
 * 回傳 flat array：out[edge_idx * layers + (l-1)] = 邊在層 l 的 ratio。
 * 呼叫者負責 free（NULL 表示 m=0 或 layers<1）。
 * ──────────────────────────────────────────────────────────── */
...
/* 單層便利 wrapper（內部呼叫 compute_external_thresholds 計算 1..k） */
double compute_external_threshold(const Graph *g, int k, int n_random);

/*
 * 一次 pass 計算 layer 1..layers 的所有 R1（外部）門檻
 * 對齊 Python HETA：T_E^k = mean_r(μ_r^k) + 2 × mean_r(σ_r^k)，clamp ≤ 1.0
 * out_thresholds：呼叫者預先分配大小至少 layers+1 的陣列
 *   out_thresholds[k] = T_E^k，k = 1..layers；out_thresholds[0] 不使用
 */
