 ...
 */
 ...
 HetaResult *heta_run(const Graph *g, int n_random) {
     ...
     /* ── Step 2：一次 pass 計算所有層的外部閾值 T_E^k ──
      * 對齊 Python HETA：對每個隨機網路各層用 Welford 算 (μ, σ)，再對 N
      * 個隨機網路的 μ 取平均、σ 取平均，最後合成 T_E^k 並 clamp ≤ 1.0。
      *
      * compute_external_thresholds 內部呼叫 compute_all_link_ratios（即
      * Python 的環狀分層演算法），與 Phase 2 取得 ratio 的方式一致。
      */
     result->ext_th = (double *)xmalloc((kmax + 1) * sizeof(double));
     result->ext_th[0] = 0.0;   /* 佔位，不使用 */

     fprintf(stdout, "[heta] Computing T_E^1..%d (n_random=%d)...\n",
             kmax, n_random);
     compute_external_thresholds(g, kmax, n_random, result->ext_th);
     ...
     /* ── Step 3.2：逐層識別 bond links 與 local bridges ──
      *
      * 對齊 Python HETA：先一次性算好原圖所有邊在所有層的 ratio
      * （即 Python 的 compute_link_property 結果），Phase 2 直接查表。
      * edges[i] 的 ratio at layer k = orig_ratios[i * kmax + (k-1)]。
      *
      * edges[] 與 compute_all_link_ratios 都是 CSR 順序 (u<v) 枚舉，
      * 所以 i 同時是 edges 與 ratios 的索引。
      */
     double *orig_ratios = compute_all_link_ratios(g, kmax);
     if (!orig_ratios && m > 0 && kmax >= 1) {
         fprintf(stderr, "[heta] compute_all_link_ratios returned NULL\n");
         exit(1);
     }
     ...
     for (int k = 1; k <= kmax; k++) {
         ...
         for (int i = 0; i < m; i++) {
             ...
             double ratio = orig_ratios[(size_t)i * kmax + (k - 1)];
