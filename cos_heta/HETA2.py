...
def edge_type_identification(g, kmax, ext_dic, return_all_list=False):
    ...
    for i in range(kmax):
        ...
        if len(Rnextstep)==0:
            ...
            T_ink = np.mean(Rnextstep) - np.std(Rnextstep)
            if T_ink<0:
                T_ink = 0.0
            for e in nextstep:
                u,v,w = e
...
def processing_link_property(iter_item):
    ...
    for i in range(sp):
        ...
        if len(common_nodes)==0:
            ...
            # cos-HETA: 分母由 min 改為幾何平均 sqrt((d_u-1)(d_v-1))
            part1_a = math.sqrt(len(c.nodes[s][l]  ) * len(c.nodes[t][l])  )
            part2_a = math.sqrt(len(c.nodes[s][l]  ) * len(c.nodes[t][l-1]))
            part3_a = math.sqrt(len(c.nodes[s][l-1]) * len(c.nodes[t][l])  )
...
def compute_link_property(g, sp):
    ...
    for layer in range(1, sp+1):
        ...
        for u1,v1 in tqdm(g.edges(), desc='layer: '+str(layer)):
            ...
            # cos-HETA: 分母由 min 改為幾何平均 sqrt((d_u-1)(d_v-1))
            part1_a = math.sqrt(uf_l * vf_l)
            part2_a = math.sqrt(uf_l * vf_k)
            part3_a = math.sqrt(uf_k * vf_l)
            denominator_a = float(part1_a + part2_a + part3_a)
    ...
    for layer in range(1, sp+1):
        l = str(layer)
        #g.graph[GRAPH_KEY_AVG_COMMON_NODES + l] = np.mean(g.graph[GRAPH_KEY_COMMON_NODES_LIST + l])
        #g.graph[GRAPH_KEY_STD_COMMON_NODES + l] = np.std( g.graph[GRAPH_KEY_COMMON_NODES_LIST + l])
        g.graph[GRAPH_KEY_AVG_COMMON_NODES + l] = np.mean(common_nodes_list[GRAPH_KEY_COMMON_NODES_LIST + l])
        g.graph[GRAPH_KEY_STD_COMMON_NODES + l] = np.std(common_nodes_list[GRAPH_KEY_COMMON_NODES_LIST + l])
...
def get_external_threshold(g, kmax, times, random_pre):
    ...
    if g.number_of_edges()>2:
        ...
        for i in range(kmax):
            l = str(i+1)
            ext = np.mean(rgmeans[l]) + np.mean(rgstds[l])
            if ext>1:
                ext_threshold[l] = 1.0
            else:
                ext_threshold[l] = ext
